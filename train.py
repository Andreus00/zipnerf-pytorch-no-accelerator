...
def main(unused_argv):
    ...
    print(f'Number of parameters being optimized: {num_params}')    # TODO: use a logger
    ...
    print("Begin training...")  # TODO: use a logger
    step = init_step + 1
    total_time = 0
    total_steps = 0
    reset_stats = True
    ...
    with logging_redirect_tqdm():
        tbar = tqdm(range(init_step + 1, num_steps + 1),
                    ...
                    # disable=not accelerator.is_main_process)  # TODO: check if this can actually be disabled.
        for step in tbar:
            try:
                batch = next(dataiter)
            ...
            # accelerator automatically handle the scale    # TODO: check if this is a problem when using loss.backward()
            ...
            if step == init_step + 1 or step % config.print_every == 0:
                ...
                print(f'{step}' + f'/{config.max_steps:d}:' +
                            f'loss={avg_loss:0.5f},' + f'psnr={avg_psnr:.3f},' +
                            f'lr={learning_rate:0.2e} | ' +
                            ','.join([f'{k}={s}' for k, s in str_losses.items()]) +
                            f',{rays_per_sec:0.0f} r/s')    # TODO: use a logger

                # Reset everything we are tracking between summarizations.
                reset_stats = True

            if step > 0 and step % config.checkpoint_every == 0: # and accelerator.is_main_process:     # TODO: this should not break the code. Check
            ...
            if config.train_render_every > 0 and step % config.train_render_every == 0:
                ...
                print(f'Eval {step}: {eval_time:0.3f}s, {rays_per_sec:0.0f} rays/sec')  # TODO: use a logger
                print(f'Metrics computed in {(time.time() - metric_start_time):0.3f}s') # TODO: use a logger
                for name, val in metric.items():
                    if not np.isnan(val):
                        print(f'{name} = {val:.4f}')    # TODO: use a logger
                ...
                with tqdm.external_write_mode():
                    print(f'Visualized in {(time.time() - vis_start_time):0.3f}s')  # TODO: use a logger
                if config.rawnerf_mode:
    ...
    if config.max_steps > init_step: # and accelerator.is_main_process: # TODO: this should not break the code. Check
        print('Saving last checkpoint at step {} to {}'.format(step, config.checkpoint_dir))    # TODO: use a logger
        checkpoints.save_checkpoint(config.checkpoint_dir,
                                    model, optimizer, step,
                                    config.checkpoints_total_limit)
    print('Finish training.')   # TODO: use a logger
